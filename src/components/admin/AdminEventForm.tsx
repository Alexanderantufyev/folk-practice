...
export function AdminEventForm({ open, date, event: initialEvent, onClose }: Props) {
  const { saveEvent, deleteEvent } = useStore()
  const [form, setForm] = useState<ClassEvent>(() => newEvent(date))
  const [newImageUrl, setNewImageUrl] = useState('')
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (open) {
      setForm(initialEvent ?? newEvent(date))
      setNewImageUrl('')
      setSaving(false)
    }
  }, [open, date, initialEvent?.id])

  const patch = (p: Partial<ClassEvent>) => setForm(f => ({ ...f, ...p }))
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 z-[55] w-full max-w-sm bg-white shadow-2xl flex flex-col overflow-hidden"
          >
